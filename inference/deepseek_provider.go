...
func (p *DeepseekProvider) PrepareRequestWithMessages(messages []gollm_types.MemoryMessage, options map[string]interface{}) ([]byte, error) {
	...
	if mtVal, ok := options["max_tokens"]; ok {
		switch mt := mtVal.(type) {
		case int: if mt > 0 { req.MaxTokens = mt }
		case float64: if mt > 0 { req.MaxTokens = int(mt) }
		}
	}
	if tVal, ok := options["temperature"].(float64); ok { req.Temperature = &tVal }
	if pVal, ok := options["top_p"].(float64); ok { req.TopP = &pVal }
	if sVal, ok := options["seed"]; ok {
		switch s := sVal.(type) {
		case int: seed64 := int64(s); req.Seed = &seed64
		case int64: req.Seed = &s
		case float64: seed64 := int64(s); req.Seed = &seed64
		}
	}
	if stopVal, ok := options["stop"].([]string); ok { req.Stop = stopVal }
	if toolsVal, ok := options["tools"].([]utils.Tool); ok {
		apiTools, err := convertToolsToDeepseek(toolsVal); if err == nil { req.Tools = apiTools }
	}
	if toolChoiceVal, ok := options["tool_choice"]; ok {
		choice := getDeepseekToolChoice(toolChoiceVal)
		if choiceStr, ok := choice.(string); ok { req.ToolChoice = choiceStr
		} else { req.ToolChoice = "auto" }
	}
	if stream, ok := options["stream"].(bool); ok && stream { req.Stream = true }
	if schemaVal, ok := options["_schema_internal"].(interface{}); ok {
		if schemaPtr, ok := schemaVal.(*jsonschema.Schema); ok {
			req.ResponseFormat = &struct { Type string `json:"type"`; JSONSchema *struct { Name string `json:"name"`; Schema *jsonschema.Schema `json:"schema"`; Strict bool `json:"strict,omitempty"` } `json:"json_schema,omitempty"` }{ Type: "json_object", JSONSchema: &struct { Name string `json:"name"`; Schema *jsonschema.Schema `json:"schema"`; Strict bool `json:"strict,omitempty"` }{ Name: "structured_output", Schema: schemaPtr } }
		}
	}
...
func (p *DeepseekProvider) setOptionInternal(key string, value interface{}) {
	...
	switch key {
	case "api_key":
		if v, ok := value.(string); ok { p.apiKey = v }
	case "model":
		if v, ok := value.(string); ok { p.model = v }
	case "max_tokens":
		switch v := value.(type) {
		case int: if v > 0 { p.maxTokens = v }
		case float64: if v > 0 { p.maxTokens = int(v) }
		}
	case "temperature":
		if v, ok := value.(float64); ok { p.temperature = &v }
	case "top_p":
		if v, ok := value.(float64); ok { p.topP = &v }
	case "seed":
		switch v := value.(type) {
		case int: seedVal := int64(v); p.seed = &seedVal
		case int64: p.seed = &v
		case float64: seedVal := int64(v); p.seed = &seedVal
		}
	default:
